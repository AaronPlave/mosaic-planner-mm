...
-64 bit OS compatibility + testing on different systems. Fairly confident that everything will run on 64 bit, although there will inevitably be some driver or library that won't work or will prove difficult to find. If you decide to make the leap, be aware that there are two version options in terms of Python -- 32 bit and 64 bit, both which run on a 64 bit system. If you decide to go for 32 bit, you're really no better off than you are just working on a 32 bit system since your memory is still limited (although this software is not very memory intensive), and will have to struggle through some extra setup. If you choose to do a 64 bit version of Python, you'll have to find all the libraries in 64 bit, which can be challenging. For example, unless you want to rewrite Mosaic Planner using a different imaging library, you'll have to either compile a 64 bit version of the right version of PIL for the right version of Python, OR use Pillow, which is a more updated version of PIL, which isn't updated much anymore.
